...
EOM_RHS = tau + k_p*(theta1_dot_goal-theta1_dot) + k_i*(theta1_dot_goal-theta1_dot)
print("RHS temp: ",EOM_RHS.subs({theta1:0,theta1_dot:0,tau:tau_float,F_a:Fa,a_1:a1,a_2:a2,theta1_dot_goal:theta1Dot_goal,k_p:kp,k_i:ki}))
print("")
...
#   EOM SOLUTION
#*********************************
# EOM_LHS_sub = EOM_LHS.subs({theta1_dot:0,theta1:0})
# EOM_RHS_sub = EOM_LHS.subs({theta1_dot:0,theta1:0})


# EOM_LHS_sub = EOM_LHS.subs({theta1_dot:y0[0],theta1:y0[-1],
#                             Icm_a1:Icma1,Icm_a2:Icma2,m_a1:ma1,m_a2:ma2,
#                             m_x:mx,g_s:g,tau:tau_float,a_1:a1,a_2:a2})
# EOM_RHS_sub = EOM_RHS.subs({theta1_dot:y0[0],theta1:y0[-1],tau:tau_float,F_a:Fa})
# print("EOM LHS sub: ",EOM_LHS_sub)
# print("EOM RHS sub: ",EOM_RHS_sub)
...
input_torque = np.zeros_like(t)
input_torque[0] = EOM_RHS.subs({theta1:y0[0],theta1_dot:y0[1],tau:tau_float,F_a:Fa,a_1:a1,a_2:a2,theta1_dot_goal:theta1Dot_goal,k_p:kp,k_i:ki})
...
for i in range(len(t)-1):
    ...
    integral_term = integral_term + (theta1_dot_goal-theta1_dot)
    EOM_RHS = tau + k_p*(theta1_dot_goal-theta1_dot) + k_i*integral_term
...
colnames = ['time', 'theta1_dot_mat', 'inputTorque']
...
inputTorque_mat = result_matlab.inputTorque.tolist()
...
plt.title(r"Crank speed vs time - ($ \dot \theta_1$ vs time)",fontsize=20)
plt.plot(t,theta1_dot_res,linewidth=2)
plt.tight_layout()
plt.subplot(2,1,2)
plt.title("Piston position vs time - X vs time",fontsize=20)
plt.xlabel('time [s]')
plt.ylabel('piston position [m]')
plt.plot(t,x_piston_position,linewidth=2)
plt.tight_layout()

plt.figure(3)
plt.xlabel("time [s]")
plt.ylabel("crank speed [rad/s]")
plt.title(r"Crank speed vs time - ($ \dot \theta_1$ vs time) - Matlab Benchmark",fontsize=20)
...
plt.title("Input torque vs time",fontsize=20)
plt.plot(t,input_torque,label="python",linewidth=2)
plt.plot(time_mat,inputTorque_mat,label="matlab",linewidth=2)
plt.legend(loc="best",fontsize=20)
plt.tight_layout()
plt.show()
